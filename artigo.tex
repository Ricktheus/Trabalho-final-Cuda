...
desprezíveis ($<0{,}001$~s). A soma das etapas ($\approx 3{,}11$~s) é
ligeiramente inferior ao tempo total de GPU da Tabela~\ref{tab:resultados}
($3{,}24$~s): a diferença de $\approx 0{,}12$~s ($\approx 3{,}7\%$)
corresponde a \emph{overhead} fora dessas etapas (alocações, \emph{kernel}
do caso analítico, cópias \emph{Device-to-Host} e reduções no \emph{host}).
