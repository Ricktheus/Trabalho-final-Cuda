...
Dunn e Silhueta exigem todas as $N(N-1)/2$ distâncias par-a-par
($\approx 5\times10^{9}$ pares distintos para $N=100.000$), levando a um
custo de tempo $O(N^2 D)$; como na formulação \emph{matrix-free} cada
distância é recomputada sob demanda, isso equivale a $\sim\!10^{10}$
avaliações de distância. Além do tempo, a abordagem ingênua que
...
conforme a Tabela~\ref{tab:memoria}: para $N=50.000$ (20~GB) já excede tanto
a memória da GPU utilizada (16~GB) quanto os $\approx\!12$~GB de RAM do
ambiente, e para $N=100.000$ (80~GB) torna-se totalmente inviável --- além
de ultrapassar o limite de indexação por inteiro de 32 bits em $N\cdot N$.
Recomputando as distâncias sob demanda dentro dos \emph{kernels}, a memória
cai para $O(N\cdot D)$ (poucos megabytes), ao custo de recalcular distâncias
--- troca amplamente vantajosa, pois é o que viabiliza a escala.
...
$N\le 32000$ e 2 para os maiores); o desvio-padrão correspondente acompanha
o código-fonte e é usado na discussão de significância. Compilação:
...
A Figura~\ref{fig:breakdown} decompõe o tempo de GPU nas \textbf{quatro
etapas instrumentadas} (H2D, Dunn, Silhueta e Davies-Bouldin). Dunn e
Silhueta --- as métricas que dependem de distâncias par-a-par ---
concentram \textbf{$\approx 99{,}95\%$} desse tempo de \emph{kernel} (em
$N=100.000$, $1{,}62$~s e $1{,}49$~s, respectivamente), enquanto
Davies-Bouldin e a transferência \emph{Host-to-Device} (H2D) são
desprezíveis ($<0{,}001$~s). A soma das quatro etapas ($\approx 3{,}11$~s) é
inferior ao tempo total reportado na Tabela~\ref{tab:resultados} ($3{,}24$~s)
porque este último inclui ainda o tempo de \textbf{leitura do \emph{dataset}
no \emph{host}} ($\approx 0{,}12$~s em $N=100.000$, ou $\approx 3{,}7\%$), que
não é uma etapa de GPU e por isso não figura na decomposição.
...
Três aspectos merecem destaque. Primeiro, o \emph{speed-up} foi medido
também contra a \textbf{CPU paralela} (OpenMP), e não apenas contra um único
núcleo. Cabe a ressalva de que as 2 \emph{vCPUs} do Google Colab
compartilham um mesmo núcleo físico (dois \emph{hyperthreads}): para uma
carga \emph{compute-bound} como esta, o OpenMP quase não acelera em $N$
grande ($80{,}61\pm1{,}17$~s contra $79{,}23\pm1{,}78$~s em $N=100.000$, um
ganho de apenas $\approx\!1{,}02\times$, dentro do desvio-padrão), o que
aproxima SU$_1$ e SU$_\Omega$. O resultado robusto, portanto, é que mesmo o
\emph{melhor} desempenho de CPU disponível na plataforma permanece
$\approx\!24\times$ atrás da GPU; aferir o \emph{speedup} de escalabilidade
forte do OpenMP exigiria uma CPU com mais núcleos físicos. Segundo, os
ganhos são \textbf{conservadores}: a T4 é
